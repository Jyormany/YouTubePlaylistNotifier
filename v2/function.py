...
#チャンネル登録者数取得関数
def getSubscriversCount(channelId:str, apiKey:str) -> int:
    ...
    #GET送信先URL
    url = reqUrl+'?key='+apiKey+'&part='+part+'&id='+channelId+'&maxResults='+maxResults
    #urlから取得して返す
    resRaw = getHttpJson(url)
    #登録者数のみ取り出し
    res = resRaw['items'][0]['statistics']['subscriberCount']
